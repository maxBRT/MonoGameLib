            ...
            using (XmlReader reader = XmlReader.Create(stream))
            {
                ...
                if (tilesetElement == null)
                {
                    System.Diagnostics.Debug.WriteLine(
                        $"Failed to load tileset from {filePath}. Tileset element not found.");
                    return null;
                }
                ...
                {
                    System.Diagnostics.Debug.WriteLine(
                        $"Failed to load tileset from {filePath}. Region attribute not found.");
                    return null;
                }
                ...
                string tileWidthString = tilesetElement.Attribute("tileWidth")?.Value;
                if (string.IsNullOrEmpty(tileWidthString))
                {
                    System.Diagnostics.Debug.WriteLine(
                        $"Failed to load tileset from {filePath}. tileWidth attribute not found.");
                    return null;
                }

                string tileHeightString = tilesetElement.Attribute("tileHeight")?.Value;
                if (string.IsNullOrEmpty(tileHeightString))
                {
                    System.Diagnostics.Debug.WriteLine(
                        $"Failed to load tileset from {filePath}. tileHeight attribute not found.");
                    return null;
                }

                int tileWidth = int.Parse(tileWidthString);
                int tileHeight = int.Parse(tileHeightString);
                ...
                {
                    System.Diagnostics.Debug.WriteLine(
                        $"Failed to load tilemap from {filePath}. Tiles element not found.");
                    return null;
                }
